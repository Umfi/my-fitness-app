<template>
  ...
  <ion-content fullscreen class="ion-padding" v-show="!loading">
    <div v-if="!workoutFinished">
      ...
      <ion-item lines="none">
        ...
        <ion-select multiple="true" v-model="muscles">
  ...
  methods: {
    ...
    async trackWorkoutAction() { 
        ...
        const newEvent = await storeWorkout({
          date: item.date,
          shoulders: (this.muscles.includes("shoulders") ? 1 : 0),
          chest: (this.muscles.includes("chest") ? 1 : 0),
          back: (this.muscles.includes("back") ? 1 : 0),
          arms: (this.muscles.includes("arms") ? 1 : 0),
          abs: (this.muscles.includes("abs") ? 1 : 0),
          legs: (this.muscles.includes("legs") ? 1 : 0),
          cardio: (this.muscles.includes("cardio") ? 1 : 0),
        });
      
        if (newEvent != null) {
          const aW = await storeAdvancedWorkout({
            date: item.date,
  ...
  watch: {
    muscles(newVal: Array<string>) {
      ...
      if (newVal.includes("shoulders")) {
        this.updateMuscleImage(document.getElementById("Deltoids"), true);
      }

      if (newVal.includes("chest")) {
        this.updateMuscleImage(document.getElementById("Pectorals"), true);
      }

      if (newVal.includes("back")) {
        this.updateMuscleImage(document.getElementById("Trapezius"), true);
        this.updateMuscleImage(document.getElementById("Lats"), true);
      }

      if (newVal.includes("arms")) {
        this.updateMuscleImage(document.getElementById("Biceps"), true);
        this.updateMuscleImage(document.getElementById("Triceps"), true);
        this.updateMuscleImage(document.getElementById("Forearms"), true);
      }

      if (newVal.includes("abs")) {
        this.updateMuscleImage(document.getElementById("Abs"), true);
        this.updateMuscleImage(document.getElementById("Obliques"), true);
      }

      if (newVal.includes("legs")) {
        this.updateMuscleImage(document.getElementById("Quads"), true);
        this.updateMuscleImage(document.getElementById("Adductors"), true);
        this.updateMuscleImage(document.getElementById("Glutes"), true);
        this.updateMuscleImage(document.getElementById("Hamstrings"), true);
